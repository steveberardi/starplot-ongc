...
def read_csv():
    ...
    gdf = gpd.GeoDataFrame(
        df,
        geometry=gpd.points_from_xy(df.ra, df.dec),
        crs=CRS,
    )
...
def walk_files(path=DATA_PATH / "outlines"):
...
def dsos_all():
    ...
    for f in walk_files():
        ...
        if not ic and not ngc:
            ...
            gdf.loc[name, "ra"] = round(centroid.x, 4)
            gdf.loc[name, "dec"] = round(centroid.y, 4)
